...
def set_color_hier(label_nodes: Union[LabelNode, Iterable[LabelNode]], colors: Optional[List[str]]=None, depth_off:int=0) -> None:
...
class QA(NamedTuple, SimpleRepr):
    ...
    def to_json(self, exclude_extras: bool=True) -> Dict[str, Any]:
        return OrderedDict(question=self.question.to_json(exclude_extras), answer=self.answer.to_json(exclude_extras))
...
class Context(SimpleRepr):
    ...
    """
    def __init__(self, fpath: str, doc_id: int, doc_title: str,
                 context_id: int, text: str, ner: List[Label] = list(),
    ...
    for context in context_it:
        ...
        if context.fpath != cur_fpath:
            ...
            if context.doc_id not in articles_dict:
                articles_dict[context.doc_id] = dict(
                    id_doc = context.doc_id,
                    title = context.doc_title,
                    contexts = list()
                )
            articles_dict[context.doc_id]["contexts"].append(dict(
...
if __name__ == "__main__":
    # ll = [Label(0,5, "B"), Label(15,17, "E"), Label(0, 15, "A"), Label(10,13, "D"), Label(8,14, "C")]
    # lnodes = LabelNode.from_labels(ll)
    # set_color_hier(lnodes)
    # txt = "".join(chr(97+i) for i in range(26))
    # print(decorate(txt, labels=lnodes))
    # exit()

    dir_path = path.join(DATA_PATH, "const_json")
    jl = json_loader(dir_path)
    fpath, fcontent = next(jl)
    fpath, fcontent = next(jl)
    doc = fcontent[0]
    para = doc["contexts"][0]
    cont = Context.from_json(dict(
        fpath=fpath,
        doc_id=doc["id_doc"],
        doc_title=doc["title"],
        context_id=para["id_context"],
        text=para["text"],
        ner=para["entities"],
        constituents=para["constituency"]
    ))
    cont.set_color_hier("constituents")
    print(decorate(cont.text, cont.constituents))
