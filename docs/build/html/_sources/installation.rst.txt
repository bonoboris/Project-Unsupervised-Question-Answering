...
---------------------------------
Follow the previously described steps but install whatever version of ``tensorflow==2.X``
you want to use.
...
If both ``SpaCy`` and ``Benepar`` are installed with GPU acceleration support
then adding:::
...
before models loadings should be enough to use GPUs.